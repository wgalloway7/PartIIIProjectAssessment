...
function run_parallel_energy(lattice::Lattice, beta_values::Vector{Float64}, l::Int64,
                             ...
                             move::String = "single flip", do_plot::Bool = false, tau_vector_file::String = "tau_vector.csv")
    println("started")
...
for (L, l_values) in L_l_pairs
    l_values = [1]
    tau_vector_file = "zL=$(L)_l=1_autocorrelation_E_tau.csv"
    println(tau_vector_file)
    lattice = Lattice(L, tau_vector_file)
    for l in l_values
        run_parallel_energy(lattice, beta_values, l, p_values, "energy_$(L)_$(l)_", copies, move, true, tau_vector_file)
    end
end
