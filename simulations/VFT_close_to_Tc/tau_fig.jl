...
Tf_vals = []
VFT_fit_vals = [Float64[], Float64[], Float64[], Int64[]]
CSD_fit_vals = [Float64[], Float64[], Int64[]]
...
for (i, l) in enumerate(l_values)
    ...
    valid = T_values .> Tc + 1e-6
    T_fit = T_values[valid]
    tau_fit = tau_mask[valid]
    println(tau_mask)
    println(T_fit)

    CSD_guess = [1.0, 2.0, 0.1]
    ...
    T_array = range(minimum(T_values), maximum(T_values), length = 200)

    if l != 1
        plot!(p, T_values, log10.(tau_mask), label=latexstring("l = $l"), color=colors[i])
        plot!(p, T_array, VFT(T_array, fit.param), color=colors[i], linestyle=:dash, label="")
    else

        plot!(p, T_values, log10.(tau_mask), label=latexstring("l = $l"), color= :red)
        plot!(p, T_array, VFT(T_array, fit.param), color= :red, linestyle=:dash, label="")
    end
